...
describe('describeEffectiveAiEndpoint', () => {
  ...
  it('an http endpoint counts only under the provider that uses it', () => {
    ...
    // 'stale-provider' stands in for an id persisted before a provider was
    // removed (§2.218): it must not be treated as OpenAI-compatible either.
    for (const provider of ['gemini-api', 'anthropic-api', 'stale-provider', undefined]) {
  ...
  it('isOpenAiCompatibleProvider names the one provider with a configurable endpoint', () => {
    ...
    for (const p of ['gemini-api', 'anthropic-api', 'stale-provider', '', undefined]) {
