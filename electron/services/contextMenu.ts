 ...
 *     See {@link reportContextMenuFailure}.
 *
 ...
 *   2. edit items — the full Cut/Copy/Paste/Select All set in an editable
...
/**
 * Which step of menu construction failed. Code-authored: the value is set by
 * this file as the handler advances, never derived from the error, so it can
 * localise a fault without describing what the step was working on.
 */
export type ContextMenuPhase = 'plan' | 'labels' | 'build' | 'popup'
...
export function attachContextMenu(win: BrowserWindow, deps: ContextMenuDeps): void {
  win.webContents.on('context-menu', (_event, params) => {
    ...
    try {
      ...
      phase = 'build'
      const menu = Menu.buildFromTemplate(plan.map(item => toTemplateItem(item, labels, deps)))
