...
describe('AiPrivacyPanel — aggregate table', () => {
  ...
  it('renders "n/a" for cost_usd=null', async () => {
    setupInvokeWithData([makeAggregateRow({ provider: 'anthropic-api', costUsd: null })], [])
    renderPanel()
    await expandPanel()
    expect(screen.getByText('n/a')).toBeInTheDocument()
  })

  // §2.218 — THE APPEND-ONLY LOG OUTLIVES THE PROVIDER ENUM. Removing a
  // provider from the live union must not make the rows it wrote unrenderable:
  // the ledger and the audit log both keep `subscription` rows with a null cost,
  // and a compromised or over-eager "validate history against the enum" change
  // would blank the user's own audit trail — the artifact the whole
  // verifiable-privacy posture rests on. Both surfaces are covered: the
  // aggregate table and a full audit row.
  it('renders history written by a REMOVED provider (aggregate + audit row)', async () => {
    setupInvokeWithData(
      [makeAggregateRow({ provider: 'subscription', requests: 3, costUsd: null })],
      [makeAuditRow({ id: 99, provider: 'subscription', costUsd: null, model: 'claude-sonnet-4-5' })],
    )
    renderPanel()
    await expandPanel()
    // The removed id is displayed verbatim in both tables, not dropped, not
    // replaced with a fallback label.
    expect(screen.getAllByText('subscription').length).toBeGreaterThanOrEqual(2)
    // Null cost still reads as "we do not know", never as $0.00.
