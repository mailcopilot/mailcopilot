...
/**
 * The providers that actually own a stored API key. `subscription` does not —
 * it authenticates through the Claude CLI.
 */
 ...
 * REQUIRES a provider, and `subscription` has no key to delete, so both cases
 ...
 * nothing is deleted for `subscription` or for "no provider", which have no key
 * to lose. Rejections propagate: the caller reports the failure instead of
 * resetting the UI as if the key were gone.
 */
