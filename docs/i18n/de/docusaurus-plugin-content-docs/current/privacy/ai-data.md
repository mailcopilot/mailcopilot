...
[Schnellaktionen beim Verfassen](../ai-assistant#schnellaktionen-beim-verfassen) schreiben den von Ihnen selbst geschriebenen Text im Verfassen-Fenster um (Verbessern / Kürzen / Förmlich). Sie folgen denselben Schutzmaßnahmen wie der Rest des KI-Assistenten:
...
- **Keine stille Ersetzung.** Eine Umformulierung wird nur als Vorher/Nachher-Vergleich angezeigt. Ihr Entwurfstext wird erst geändert, nachdem Sie explizit **Ersetzen** oder **Unter meinem Text einfügen** angeklickt haben -- ein Klick auf **Abbrechen** oder das Schließen des Vergleichs lässt Ihren Entwurf unverändert, und es wird nichts weiter gesendet.
...
- **Schutz vor gleichzeitigen Änderungen.** Wenn Sie weitertippen, während eine Umformulierung erzeugt wird, wird **Ersetzen** deaktiviert, sobald die Umformulierung zurückkommt, damit sie nicht den Text überschreiben kann, den Sie inzwischen getippt haben; **Unter meinem Text einfügen** bleibt verfügbar.
...
- **Auditierte Erzeugungen.** Jede Umformulierung schreibt einen Eintrag im [KI-Auditprotokoll](#das-auditprotokoll) mit dem Ziel `quick_action`; das konkret verwendete Preset wird im Telemetrie-Span erfasst, nicht im Audit-Eintrag.
...
[AI Proofread](../ai-assistant#ai-proofread) prüft den von Ihnen selbst geschriebenen Text und listet vorgeschlagene Korrekturen einzeln auf, statt wie die Presets oben den gesamten Text umzuschreiben. Es folgt denselben Schutzmaßnahmen wie der Rest des KI-Assistenten:

- **Standardmäßig deaktiviert, pro Konto.** Es wird nichts zur Prüfung gesendet, solange Sie **AI Proofread** nicht für dieses spezifische Postfach in der Tabelle [KI-Funktionen je Postfach](../ai-assistant#ki-funktionen-je-postfach) aktivieren. **Anders als bei den meisten anderen KI-Freigaben wird die Schaltfläche dabei nicht ausgeblendet** -- sie bleibt sichtbar, in einem sichtbar gesperrten Zustand, mit einem Hinweis beim Überfahren mit der Maus, wo sie eingeschaltet werden kann. Ein Klick darauf sendet nichts, solange sie gesperrt ist; MailCopilot lehnt zudem unabhängig auf der Verbindung zum KI-Anbieter ab, wenn die Einstellung deaktiviert ist, sodass die Freigabe zweimal geprüft wird, nicht nur durch die graue Farbe der Schaltfläche.
- **Nur Ihr eigener Text verlässt das Gerät -- bei Entwürfen, die MailCopilot selbst verfasst hat.** Dieselbe Grenze für den eigenen Text wie bei den Schnellaktionen beim Verfassen gilt auch hier: Die zitierte Nachricht, die Weiterleitungs-Kopfzeile und die Signatur sind von der Prüfung ausgeschlossen. Siehe [Schnellaktionen beim Verfassen](#schnellaktionen-beim-verfassen) oben dafür, wie diese Grenze erkannt wird und wo sie versagen kann.
- **Keine stille Ersetzung.** Vorschläge werden nur als Liste gezeigt, die Sie einzeln annehmen; der Entwurf ändert sich erst, nachdem Sie explizit auf **Accept** (oder **Accept all**) und dann auf **Apply selected** geklickt haben.
...
- **Auditierte Erzeugungen.** Jede Prüfung schreibt einen Eintrag im [KI-Auditprotokoll](#das-auditprotokoll).
- **Anbieterauswahl.** AI Proofread nutzt Ihren konfigurierten **API-Key-Anbieter** (Anthropic, OpenAI-kompatibel oder Google Gemini).
- **Budgetbewusst.** Wenn das tägliche KI-Budget erreicht wurde, wird die Prüfung sauber abgelehnt -- siehe [AI Proofread](../ai-assistant#ai-proofread) für das, was Sie in diesem Fall sehen.
- **Telemetrie enthält keine Nachrichteninhalte.** Das pseudonyme Nutzungsereignis, das bei jeder Prüfung erfasst wird, enthält nur die Anbieterkennung, ob das Modell lokal lief, Tokenzahlen, Latenz und eine gebündelte Fehlerklasse -- niemals den Entwurfstext oder die Vorschläge selbst.
...
- **Standardmäßig deaktiviert, pro Konto -- keine eigene Einstellung.** Die Entwurfsübersetzung wird über dieselbe Freigabe **AI Translate** wie die Nachrichtenübersetzung gesteuert, aktiviert je Postfach in der Tabelle [KI-Funktionen je Postfach](../ai-assistant#ki-funktionen-je-postfach); es gibt nichts Zusätzliches zu aktivieren. **Anders als bei den meisten anderen KI-Freigaben werden Liste und Schaltfläche dabei nicht ausgeblendet** -- sie bleiben sichtbar, die Schaltfläche in einem sichtbar gesperrten Zustand, mit einem Hinweis beim Überfahren mit der Maus, wo sie eingeschaltet werden kann. Ein Klick auf die Schaltfläche sendet nichts, solange sie gesperrt ist; MailCopilot lehnt zudem unabhängig auf der Verbindung zum KI-Anbieter ab, wenn die Einstellung deaktiviert ist.
...
- **Keine stille Ersetzung.** Die Übersetzung wird nur als Vorher/Nachher-Vergleich im selben Prüfpanel gezeigt, das die Schnellaktionen beim Verfassen verwenden. Ihr Entwurfstext ändert sich erst, nachdem Sie explizit **Ersetzen** oder **Unter meinem Text einfügen** angeklickt haben.
