...
- Im **KI-Panel** (bei angezeigtem Fehler) klicken Sie auf **Anbieter wechseln**, um den aktuellen Anbieter zurueckzusetzen und einen neuen auszuwaehlen.
- In **Einstellungen > KI** klicken Sie auf **Konfiguration zuruecksetzen** neben dem Namen des aktuellen Anbieters. Dadurch wird der gespeicherte API-Schluessel geloescht und Sie koennen neu beginnen.
...
- **HTTP-Proxy** -- wenn Ihr Netzwerk einen HTTP-Proxy für den Internetzugang erfordert, geben Sie hier die Proxy-URL ein (z. B. `http://proxy.company.local:3128`). Der Proxy wird für alle KI-Anfragen verwendet. Leer lassen, wenn kein Proxy benötigt wird.
