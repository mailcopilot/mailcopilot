...
- **AI Proofread** -- aktivieren Sie „Entwuerfe mit KI auf Fehler pruefen", um im Verfassen-Fenster eine Schaltflaeche **Check writing** hinzuzufuegen. Die Schaltflaeche zeigt eine Liste vorgeschlagener Korrekturen; Sie nehmen jede einzeln an. Standardmaessig deaktiviert; wird fuer jedes Konto separat aktiviert. Siehe [AI Proofread](#ai-proofread) unten fuer Details.
...
Im Verfassen-Fenster wird über dem Nachrichtentext eine kleine Symbolleiste mit vier KI-Umformulierungsschaltflächen angezeigt: **Verbessern**, **Kürzen**, **Förmlich** und **Grammatik korrigieren**. Klicken Sie auf eine davon, damit die KI den von Ihnen selbst geschriebenen Text für dieses Ziel umschreibt.
...
2. Klicken Sie auf **Verbessern**, **Kürzen**, **Förmlich** oder **Grammatik korrigieren** in der Symbolleiste über dem Nachrichtentext.
...
4. Wählen Sie eine von drei Aktionen:
   ...
   - **An Cursor einfügen** -- den umformulierten Text an der aktuellen Cursorposition einfügen, statt Ihren Text zu ersetzen.
   - **Abbrechen** -- die Umformulierung verwerfen und den Entwurf unverändert lassen.

Ihr Entwurf wird **niemals automatisch geändert** -- die Umformulierung erscheint nur als Vorher/Nachher-Vergleich, und der Text wird erst geändert, nachdem Sie explizit **Ersetzen** oder **An Cursor einfügen** angeklickt haben.
...
**Wenn Sie weitertippen, während eine Umformulierung erzeugt wird:** Hat sich der Entwurf geändert, bis die Umformulierung zurückkommt, wird **Ersetzen** mit der Warnung **„Sie haben den Entwurf geändert, während die KI gearbeitet hat; ein Ersetzen würde diese Änderungen verwerfen. Fügen Sie stattdessen an der Cursorposition ein oder starten Sie die Aktion erneut."** deaktiviert. **An Cursor einfügen** bleibt verfügbar, da diese Aktion die Umformulierung an der aktuellen Cursorposition hinzufügt, ohne etwas von dem zu überschreiben, was Sie getippt haben.
...
1. Oeffnen Sie **Einstellungen** und gehen Sie zum Tab **KI**.
2. Suchen Sie **AI Proofread** und aktivieren Sie „Entwuerfe mit KI auf Fehler pruefen".
...
**Wenn die Funktion nicht aktiviert ist** fuer das aktuelle Konto, zeigt ein Klick auf **Check writing** die Meldung: „Turn on AI proofreading for this account in Settings to check your writing."
...
**Aktivierung.** Es gibt keine eigene Einstellung: Die Entwurfsübersetzung nutzt denselben Schalter **KI-Übersetzung** wie die [Nachrichtenübersetzung](#nachrichtenübersetzung) oben -- **Einstellungen > KI > KI-Übersetzung > Übersetzen empfangener Nachrichten und eigener Entwürfe durch die KI erlauben**, standardmäßig deaktiviert und pro Konto aktiviert.
...
3. MailCopilot zeigt die Übersetzung im selben Panel „KI-Umformulierung prüfen" wie bei den vier vordefinierten Umformulierungen, mit den Schaltflächen **Ersetzen**, **An Cursor einfügen** und **Abbrechen** -- siehe [Schnellaktionen beim Verfassen](#schnellaktionen-beim-verfassen) dafür, wie dieses Panel funktioniert. Nichts wird von selbst in Ihren Entwurf übernommen; der Text ändert sich erst, nachdem Sie explizit **Ersetzen** oder **An Cursor einfügen** angeklickt haben.
