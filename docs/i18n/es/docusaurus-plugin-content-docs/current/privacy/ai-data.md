...
Las [Acciones rápidas al redactar](../ai-assistant#acciones-rapidas-al-redactar) reescriben el texto que usted mismo ha escrito (Mejorar / Acortar / Formal / Corregir gramática) en la ventana de redacción. Siguen las mismas protecciones que el resto del asistente de IA:
...
- **Protección contra ediciones simultáneas.** Si sigue escribiendo mientras una reescritura está en curso, **Reemplazar** se deshabilita en cuanto llega la reescritura, para que no pueda sobrescribir el texto que haya escrito mientras tanto; **Insertar en el cursor** sigue disponible.
...
- **Deshabilitada de forma predeterminada, por cuenta -- sin ajuste independiente.** La traducción del borrador se controla con el mismo interruptor **Configuración > IA > Traducción con IA** que la traducción del mensaje; no hay nada adicional que activar.
...
- **Sin sustitución silenciosa.** La traducción solo se muestra como una comparación antes/después, en el mismo panel de revisión que usan las acciones rápidas al redactar. El cuerpo de su borrador solo cambia después de que haga clic explícitamente en **Reemplazar** o **Insertar en el cursor**.
