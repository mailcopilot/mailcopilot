...
MailCopilot decide se scrivere questo file una sola volta, all'avvio, in base al valore di questa impostazione in quel momento -- attivarla ha effetto solo dopo aver riavviato MailCopilot. In una copia installata dell'applicazione non esiste alcun file di log finché non avete abilitato questa opzione e riavviato almeno una volta; quindi, se state indagando su un problema, attivatela, riavviate e solo dopo provate a riprodurlo. Il file cattura esclusivamente ciò che accade nel processo principale di MailCopilot; l'output diagnostico prodotto all'interno di una determinata finestra -- ciò che vedreste nella console degli strumenti di sviluppo di quella finestra -- non viene scritto qui.
