...
L'URL base si applica solo a un provider compatibile con OpenAI -- con Gemini, Anthropic o un abbonamento Claude selezionato, l'indirizzo viene salvato ma non è di fatto usato da nessuna parte. La finestra di dialogo ne tiene conto e ti avverte di ciò che accadrà realmente una volta approvato, non di una modifica che avrebbe effetto immediato:
