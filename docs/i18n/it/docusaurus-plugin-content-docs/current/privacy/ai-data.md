...
Le [Azioni rapide nella composizione](../ai-assistant#azioni-rapide-nella-composizione) riscrivono il testo che avete scritto voi stessi (Migliora / Accorcia / Formale) nella finestra di composizione. Seguono le stesse protezioni del resto dell'assistente IA:
...
- **Nessuna sostituzione silenziosa.** Una riscrittura viene mostrata solo come confronto prima/dopo. Il corpo della vostra bozza cambia solo dopo che avete fatto clic esplicitamente su **Sostituisci** o **Aggiungi sotto il mio testo** — fare clic su **Annulla**, o chiudere il confronto, lascia la bozza invariata e non viene inviato nient'altro.
...
- **Protezione dalle modifiche concorrenti.** Se continuate a digitare mentre una riscrittura è in corso, **Sostituisci** viene disabilitato non appena la riscrittura torna disponibile, così da non poter sovrascrivere il testo digitato nel frattempo; **Aggiungi sotto il mio testo** resta disponibile.
...
- **Generazioni sottoposte ad audit.** Ogni riscrittura scrive una voce nel [registro di audit IA](#il-registro-di-audit) con `goal` impostato su `quick_action`; il preset specifico usato viene registrato nello span di telemetria, non nella voce di audit.
...
[AI Proofread](../ai-assistant#ai-proofread) controlla il testo che avete scritto voi stessi ed elenca le correzioni suggerite una alla volta, invece di riscrivere tutto il testo come i preset qui sopra. Segue le stesse protezioni del resto dell'assistente IA:

- **Disattivata per impostazione predefinita, per account.** Non viene inviato nulla per il controllo finché non abilitate **AI Proofread** per quella specifica casella nella tabella [Funzioni IA per casella](../ai-assistant#funzioni-ia-per-casella). **A differenza della maggior parte delle altre autorizzazioni IA, il pulsante non viene per questo nascosto** -- resta visibile, in uno stato visibilmente bloccato, con un suggerimento al passaggio del mouse che indica dove attivarlo. Fare clic mentre è bloccato non invia nulla; MailCopilot rifiuta inoltre in modo indipendente sulla connessione al provider IA se l'impostazione è disattivata, così l'autorizzazione viene verificata due volte, non solo tramite il colore grigio del pulsante.
- **Solo il vostro testo lascia il dispositivo, sulle bozze composte da MailCopilot.** Qui si applica lo stesso confine del testo proprio usato dalle azioni rapide nella composizione: il messaggio citato, l'intestazione di inoltro e la firma sono esclusi da ciò che viene controllato. Vedi [Azioni rapide nella composizione](#azioni-rapide-nella-composizione) qui sopra per sapere come viene rilevato quel confine e dove può non funzionare.
- **Nessuna sostituzione silenziosa.** I suggerimenti vengono mostrati solo come un elenco che accettate individualmente; la bozza cambia solo dopo che fate clic esplicitamente su **Accept** (o **Accept all**) e poi su **Apply selected**.
- **Contenuto avvolto.** Il vostro testo viene avvolto con marcatori di confine `wrapUntrusted()` prima di raggiungere il provider IA, la stessa protezione descritta in [Protezione contro l'iniezione di prompt](#protezione-contro-liniezione-di-prompt) più sotto.
- **Generazioni sottoposte ad audit.** Ogni controllo scrive una voce nel [registro di audit IA](#il-registro-di-audit).
- **Selezione del provider.** AI Proofread utilizza il vostro **provider configurato tramite chiave API** (Anthropic, compatibile con OpenAI, o Google Gemini).
- **Attento al budget.** Se il budget IA giornaliero è stato raggiunto, il controllo viene rifiutato in modo controllato — vedi [AI Proofread](../ai-assistant#ai-proofread) per cosa viene mostrato in questo caso.
- **La telemetria non contiene contenuto dei messaggi.** L'evento di utilizzo pseudonimo registrato per ogni controllo riporta solo l'identificativo del provider, se il modello è stato eseguito localmente, i conteggi dei token, la latenza e una classe di errore aggregata — mai il testo della bozza né i suggerimenti stessi.
...
- **Disattivata per impostazione predefinita, per account -- senza impostazione separata.** La traduzione della bozza è controllata dalla stessa autorizzazione **AI Translate** della traduzione del messaggio, attivata per casella nella tabella [Funzioni IA per casella](../ai-assistant#funzioni-ia-per-casella); non c'è nulla di aggiuntivo da attivare. **A differenza della maggior parte delle altre autorizzazioni IA, l'elenco e il pulsante non vengono per questo nascosti** -- restano visibili, il pulsante in uno stato visibilmente bloccato, con un suggerimento al passaggio del mouse che indica dove attivarlo. Fare clic sul pulsante mentre è bloccato non invia nulla; MailCopilot rifiuta inoltre in modo indipendente sulla connessione al provider IA se l'impostazione è disattivata.
...
- **Nessuna sostituzione silenziosa.** La traduzione viene mostrata solo come confronto prima/dopo, nello stesso pannello di revisione usato dalle azioni rapide nella composizione. Il corpo della tua bozza cambia solo dopo che fai clic esplicitamente su **Sostituisci** o **Aggiungi sotto il mio testo**.
