...
The Base URL only applies to an OpenAI-compatible provider -- with Gemini, Anthropic, or a Claude subscription selected, the address is saved but nothing is actually sent to it. The dialog takes this into account and warns you about what will actually happen once you approve, not about a change that takes effect immediately:
