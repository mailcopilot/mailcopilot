...
A small AI toolbar appears above the message body with three buttons: **Improve**, **Shorter**, and **Formal**. Click one to have the AI rewrite the text you wrote yourself -- the quoted message you are replying to, any forwarded-message header, and your signature are left untouched, for replies, forwards, and signatures that MailCopilot itself produced, and for the widespread quoting conventions other clients follow. **A draft composed in a different mail client may quote in a style MailCopilot does not recognize -- on such a draft no boundary is found, the whole body counts as your own text, and the quoted part is rewritten right along with it.** See [Compose Quick Actions](../ai-assistant#compose-quick-actions) for exactly which quoting styles are and are not recognized.

MailCopilot shows a **Review AI rewrite** panel: your own text and the rewrite appear together as one merged, scrollable passage with the edits marked in place -- removed words struck through, added words highlighted -- plus a list of the individual edits below it; plain **Before** / **After** copies stay available by expanding **Plain text**. Choose **Replace** to swap your own text with the rewrite (the quoted message and signature underneath are carried through unchanged when a boundary was found, as described above), **Add below my text** to insert it at the end of your own text, above any quoted message, forwarded header, or signature, or **Cancel** to discard the rewrite and keep your draft unchanged. The body is only modified if you choose **Replace** or **Add below my text** -- **Cancel** leaves your draft exactly as it was.

Quick Actions requires an AI provider to be configured (see [AI Assistant](../ai-assistant)) and needs some text of your own, written above any quoted message, to rewrite. **Fixing mistakes has its own, more targeted button** -- see [AI Proofread](../ai-assistant#ai-proofread) below. See [Compose Quick Actions](../ai-assistant#compose-quick-actions) for the full behavior and privacy details.

## AI Proofread

Next to the rewrite buttons, a **Check writing** button lists suggested corrections -- spelling, grammar, punctuation, and wording -- one by one, so you can accept each individually instead of rewriting the whole text. This button is **off by default, per mailbox**, and unlike the reading-side AI features it stays visible even when it is off: for a mailbox that has not opted in, it is shown locked, with a hint on hover pointing to **Settings → AI** to turn it on. See [AI Proofread](../ai-assistant#ai-proofread) for the full behavior and privacy details.

## Draft Translation

Next to the rewrite buttons, a **Translate the draft into** language picker and a **Translate** button let you write a reply in a language other than the one you typed it in. This is also **off by default, per mailbox**, sharing the same **AI Translate** opt-in as reading-side message translation; when it is off, the picker and button stay visible but locked, with a hint pointing to where to turn it on, rather than disappearing. Pick a target language -- or keep the suggestion MailCopilot may have pre-filled when you are replying, the detected language of the message you are answering -- and click **Translate**. The result appears in the same **Review AI rewrite** panel used above, with **Replace**, **Add below my text**, and **Cancel**; nothing is inserted into your draft on its own. Only the text you wrote yourself is translated -- the quoted message, forwarded header, and signature are left untouched, when a boundary is found: this uses the same detection as Quick Actions above, so a draft composed in a different mail client with an unrecognized quoting style has no boundary found and gets translated whole, quoted text included. If the provider's answer hits its own length limit, or the draft is too long to translate at all, the **Translate** button stays disabled rather than offering a retry -- another attempt would be a fresh billed request for the same result; other failures leave the button clickable again. See [Draft Translation](../ai-assistant#draft-translation) for the full behavior and privacy details.
