 ...
 *    Still not isolation, and the residue is worth naming precisely, because
 *    §2.145 changed part of it. What is now IN PLACE: an explicit bound applied
 *    before the bytes are accepted at all (`MAX_EML_PARSE_BYTES`, 100 MiB,
 *    enforced in eml.ts before dispatch — nothing larger reaches this client)
 *    and a cap on decoded output inside the parse (`EML_BODY_SOFT_CAP_BYTES`,
 *    carried per job as `maxBodyBytes`). So the parse's appetite is no longer
 *    unbounded: a message can still cost a whole worker per open attempt, but
 *    the input is bounded and so is what comes back.
 *    What is still OUTSTANDING: real memory isolation, which needs a separate
 *    PROCESS (`utilityProcess` / `child_process`, at the cost of a
 *    structured-clone hop over an IPC channel instead of a thread-local one) —
 *    filed as §2.147. Until then an allocation that escapes the V8 heap still
 *    kills the process, and the input bound is what makes that require a
 *    pathological MIME shape rather than merely a large message. Do not read
 *    the `resourceLimits` line below as if it were isolation.
...
/**
 * §2.145 — exported so `eml.test.ts` can pin the ordering against
 * `MAX_EML_PARSE_BYTES` (100 MiB), which must stay ABOVE it. The two bounds
 * answer different questions and their relative order is what keeps them
 * distinguishable to a user: the hard cap is deterministic and per message,
 * this one fires only under concurrency. Inverting them would make "too big"
 * arrive as a load-dependent, unreproducible "the message would not open".
 */
 ...
 * (`onWorkerFailure`).
 *
 * §2.145 supplied the input-size half of what bounding the hostile case needs:
 * nothing above `MAX_EML_PARSE_BYTES` (100 MiB) is dispatched here at all, so
 * this ceiling is no longer being asked to hold back an input of arbitrary
 * size. The other half — a separate PROCESS, so that an allocation escaping the
 * V8 heap cannot take the application with it — is still outstanding (§2.147).
 */
 ...
 *
 *  §2.145 — `maxBodyBytes` travels WITH the request rather than being read from
 *  a constant inside the worker: the limit is a property of one open (the user
 *  may have asked for the raised tier), and a worker that decided it for itself
 *  would silently return the first-tier body for a "show full message" click. */
export type EmlWorkerJob =
  | { type: 'parseDetails'; uid: number; raw: Uint8Array; maxBodyBytes: number }
...
class EmlParseWorkerClient {
   ...
   *
   * §2.145 sharpened, but did not change, that reasoning. "Whatever its size"
   * now has a ceiling — nothing above `MAX_EML_PARSE_BYTES` reaches this client
   * at all, the hard cap having answered it before dispatch — so the largest
   * lone request admitted here is bounded, which it was not before. This bound
   * is still about accumulation only.
   */
   ...
   *
   * Identity is (type, uid, body limit, exact bytes) — see `sameBytes` for why
   * exact. A finished job is never a candidate, so a late duplicate starts
   * fresh rather than attaching to something that will never settle again.
   *
   * §2.145 — the body limit is part of the identity, and leaving it out was the
   * bug waiting to happen: "show full message" is by construction a SECOND
   * request for bytes already being parsed, so without this it would join the
   * first-tier job and be answered with the very truncation the user clicked to
   * escape — and the click would look broken rather than slow.
   */
  private findCoalescible(request: EmlWorkerJob): Job | null {
    ...
    for (const job of candidates) {
      ...
      if (job.request.type === 'parseDetails' && request.type === 'parseDetails') {
        if (job.request.uid !== request.uid) continue
        if (job.request.maxBodyBytes !== request.maxBodyBytes) continue
      }
...
export function parseEmlDetailsInWorker(
  ...
  return client.run({ type: 'parseDetails', uid, raw, maxBodyBytes }, signal) as Promise<MessageDetails>
}
