...
async function handle(request: EmlWorkerRequest): Promise<void> {
  try {
    if (request.type === 'parseDetails') {
      // §2.145 — the decoded-body cap comes from the REQUEST, never from a
      // constant chosen here: it is a property of one open (first tier, or the
      // raised tier the user explicitly asked for), and the client is the only
      // side that knows which.
      const details = await parseEmlBufferInline(request.uid, toBuffer(request.raw), request.maxBodyBytes)
