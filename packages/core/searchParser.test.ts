...
describe('packages/core/searchParser', () => {
  ...
  it('filename: parses a single attachment filename', () => {
    const q = parseSearchQuery('filename:report.pdf')
    expect(q.filename).toEqual(['report.pdf'])
    expect(isAdvancedSearch(q)).toBe(true)
  })

  it('filename: supports quoted values', () => {
    const q = parseSearchQuery('filename:"quarterly report.xlsx"')
    expect(q.filename).toEqual(['quarterly report.xlsx'])
  })

  it('-filename: adds to notFilename', () => {
    const q = parseSearchQuery('-filename:spam.zip')
    expect(q.notFilename).toEqual(['spam.zip'])
    expect(q.filename).toEqual([])
    expect(isAdvancedSearch(q)).toBe(true)
  })

  it('filename: combines with other operators', () => {
    const q = parseSearchQuery('from:alice@a.com filename:invoice.pdf has:attachment')
    expect(q.from).toEqual(['alice@a.com'])
    expect(q.filename).toEqual(['invoice.pdf'])
    expect(q.hasAttachment).toBe(true)
  })

  it('multiple filename: values are collected', () => {
    const q = parseSearchQuery('filename:a.pdf filename:b.docx')
    expect(q.filename).toEqual(['a.pdf', 'b.docx'])
  })

  it('filename and notFilename are empty by default', () => {
    const q = parseSearchQuery('hello world')
    expect(q.filename).toEqual([])
    expect(q.notFilename).toEqual([])
  })

  it('rejects invalid calendar dates like 2024-02-31', () => {
    const q = parseSearchQuery('before:2024-02-31 after:2024-04-31')
    expect(q.before).toBeUndefined()
    expect(q.after).toBeUndefined()
  })

  it('rejects month 13 and day 0', () => {
    const q1 = parseSearchQuery('before:2024-13-01')
    expect(q1.before).toBeUndefined()

    const q2 = parseSearchQuery('after:2024-01-00')
    expect(q2.after).toBeUndefined()
  })

  it('accepts valid leap day 2024-02-29', () => {
    const q = parseSearchQuery('before:2024-02-29')
    expect(q.before).toBe('2024-02-29')
  })

  it('rejects non-leap year Feb 29', () => {
    const q = parseSearchQuery('before:2023-02-29')
    expect(q.before).toBeUndefined()
  })
})
